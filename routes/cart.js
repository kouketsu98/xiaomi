...
router.post('/list', async (ctx, next) => {
	...
	const sql = 'select T1.id,T1.pid,T1.count,T2.name,T2.avatar,T2.price,T2.cid,T2.brief from (select * from `dt_cart` where `name` = ?) T1 inner join `dt_product` T2 on T1.pid = T2.id;';
...
router.post('/list_ids', async (ctx, next) => {
	...
	var sql = 'select T1.id,T1.pid,T1.count,T2.name,T2.avatar,T2.price,T2.cid,T2.brief from (select * from `dt_cart` where `id` in (?)) T1 inner join `dt_product` T2 on T1.pid = T2.id;';
